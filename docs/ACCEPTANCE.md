...
| 项 | 结果 |
| --- | --- |
| 自动化测试 | **45/45 通过**（单元 + apply 集成 + 真实时序回归 + P0/P1/P2 闭环 + M4 Phase 1/Phase 2/Phase 3 流式/客户端/接管测试） |
...
| 模块 | 行 | 分支 | 函数 |
| --- | --- | --- | --- |
| src/client.ts | 84.72% | 80.00% | 92.00% |
| src/index.ts | 87.78% | 68.75% | 80.77% |
| src/pure.ts | 93.22% | 100% | 87.50% |
| src/stream.ts | 96.79% | 91.67% | 100% |
| src/trace.ts | 83.05% | 72.73% | 66.67% |
| **all files** | **88.48%** | **78.30%** | **87.50%** |

行覆盖 88.48% ≥ 80% 门槛；关键分支（预算守卫、静默窗时序、attach-monitor 触发、错误入账、M4 流式广播与锁状态）均有专项测试。
...
- 文件：`docs/packages/dsh-interactive-shell-0.1.1.tgz`（32926 bytes）
- SHA256：`5314C9270FF5286B1DD95FA7FE0BD99B6EAB36AADDDE72C22824874F957249FA`