...
export class DshShellPanelController {
  private readonly clients = new Map<string, TermStreamClient>()
  private readonly clientDisposers = new Map<string, () => void>()
  ...
  addSession(client: TermStreamClient): void {
    if (this.clients.has(client.sessionId)) return
  ...
  selectSession(sessionId: string): boolean {
    if (!this.clients.has(sessionId)) return false
    if (this.activeId === sessionId) return true
