...
// Type-only: augments Context with `agentPresets` (host service of the preset registry).
import type {} from '@deepseek-ai/dsh-agent-preset-registry'

/** Actionable failure text for a call whose agent resolves to no PTY registry. */
export const PTY_UNAVAILABLE_MESSAGE =
  'interactive_shell found no persistent PTY registry for this agent. Since dsh 0.1.7 the PTY family ' +
...
export const NO_AGENT_MESSAGE =
  'interactive_shell requires a calling agent: every PTY session is owner-scoped and this bridge never ' +
 ...
 */
export function resolveTerminals(ctx: Context, agent: Agent): TerminalSessionService | undefined {
 ...
 */
export function requireTerminals(ctx: Context, agent: Agent): TerminalSessionService {
  const term = resolveTerminals(ctx, agent)
  if (term === undefined) throw new Error(PTY_UNAVAILABLE_MESSAGE)
  return term
}
