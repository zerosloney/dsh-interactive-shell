...
const ANSI_REGEX = new RegExp('[\u001B\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%_~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%_~_]*)*)?\\u0007)|(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TZcf-ntqry=><~]))', 'g');
...
export class TermStreamClient {
    ...
    connectHub(hub, replay = true) {
        ...
        this.onLockRequest = (sessionId, action, userOrSummary) => {
            if (action === 'acquire')
                hub.acquireLock(sessionId, userOrSummary);
            else
                hub.releaseLock(sessionId, userOrSummary);
        };
