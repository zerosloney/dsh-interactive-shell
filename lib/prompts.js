 ...
 */
export function parseInteractivePrompt(terminalText) {
    if (!terminalText)
        return null;
 ...
 */
export function generatePromptAnswer(prompt, options = {}) {
    ...
    if (prompt.kind === 'numbered_menu' && prompt.choices && options.preferredChoice) {
        const target = prompt.choices.find((c) => c.label.toLowerCase().includes(options.preferredChoice.toLowerCase()) ||
    ...
    if (prompt.kind === 'select_menu' && prompt.choices && options.preferredChoice) {
        const targetIdx = prompt.choices.findIndex((c) => c.label.toLowerCase().includes(options.preferredChoice.toLowerCase()) ||
    ...
    if (prompt.kind === 'multi_select' && prompt.choices && options.preferredChoices && options.preferredChoices.length > 0) {
        ...
        for (const pref of options.preferredChoices) {
            const idx = prompt.choices.findIndex((c) => c.label.toLowerCase().includes(pref.toLowerCase()) ||
        ...
        if (targetIndices.size > 0) {
            ...
            for (let i = 0; i < prompt.choices.length; i++) {
                if (targetIndices.has(i)) {
                    ...
                    if (steps > 0)
                        keys += '\x1B[B'.repeat(steps);
                    else if (steps < 0)
                        keys += '\x1B[A'.repeat(Math.abs(steps));
                    currentPos = i;
                    keys += ' ';
                }
            }
