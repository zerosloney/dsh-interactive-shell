...
export function apply(ctx, config) {
    ...
    const streamHub = new StreamHub({
        ...
        onReleaseLock: (sessionId, summary) => {
            ...
            wakeAgent(state.owner, `User released control of shell session ${sessionId}`, `[interactive_shell] User released interactive control back to you for session ${sessionId} (${state.command}).${summary ? `\nUser note: ${summary}` : ''}\n\nCurrent tail:\n${tail}`);
        },
    });
    ...
    function readTail(state) {
        ...
        if (pending === 0) {
            ...
            if (lastLine !== state.seenLastLine)
                chunk = lastLine;
    ...
    function fireMonitor(state, delta) {
    ...
    function pollMonitor(state) {
        ...
        if (!triggerMatches(state.trigger, delta))
            return true;
    ...
    function probeMonitor(state) {
        ...
        if (!monitorCooldownElapsed(state.lastEventAt, Date.now(), config.monitorCooldownMs))
            return;
        if (!triggerMatches(state.trigger, page.text))
            return;
    ...
    function startPolling(state) {
        ...
        const tick = () => {
            ...
            if (state.mode === 'dispatch') {
                ...
                if (!sessions.has(state.sessionId) || !state.monitoring)
                    return;
            }
    ...
    const tool = defineTool({
        ...
        parameters: {
            ...
            mode: {
                ...
                description: 'spawn: how results come back. interactive = agent drives it; dispatch = one wake-up on exit, quiet window, or deadline; monitor = wake only when a trigger matches.',
            },
            sessionId: {
                type: 'string',
            ...
            timeoutMs: {
                type: 'integer',
                description: 'spawn in dispatch mode: absolute deadline in ms for this run (1000-3600000). Defaults to the configured dispatchTimeoutMs.',
            },
        },
        output: {
            schema: {
        ...
        async execute(args, exec) {
            const a = args;
            const sanitizeOutput = (text) => config.redactSensitiveData !== false ? redactSensitiveData(text).text : text;
            try {
                switch (a.action) {
                    case 'spawn': {
                        ...
                        if (a.timeoutMs !== undefined
                            ...
                            throw new Error(`interactive_shell timeoutMs must be between ${MIN_DISPATCH_TIMEOUT_MS} and ${MAX_DISPATCH_TIMEOUT_MS} ms`);
                        }
                        // M5: 生产安全沙箱与高危指令检查
                        ...
                        const live = listed.filter((entry) => spawnedIds.has(String(entry.sessionId)) && entry.status.kind === 'running').length;
                    ...
                    case 'send': {
                        ...
                        if (lock.state === 'user_takeover') {
                            throw new Error(`interactive-shell: session ${a.sessionId} is currently locked by user takeover (${lock.lockedBy ?? 'user'}) — wait for user to release control`);
                        }
                        const owner = requireAgent(exec.agent);
                    ...
                    case 'status': {
                        ...
                        const tail = truncateTail(term.read(owner, TerminalSessionId(a.sessionId)).text, config.outputTailBytes);
                        const exited = snap.status.kind === 'exited';
                        if (exited) {
                        ...
                        return { sessionId: a.sessionId, text: sanitizeOutput(`status=${snap.status.kind}${exited ? ` exit=${snap.status.exitCode}` : ''}\n${tail}`), exited };
                    }
                    case 'read': {
                        if (a.sessionId === undefined)
